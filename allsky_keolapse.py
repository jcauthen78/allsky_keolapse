...
ALLSKY_IMAGES = s.getEnvironmentVariable("ALLSKY_IMAGES", fatal=False) or "/home/pi/allsky/images"
base_dir = s.getSetting("imagepath") or ALLSKY_IMAGES
ALLSKY_TMP = s.getEnvironmentVariable("ALLSKY_TMP", fatal=False) or "/home/pi/allsky/tmp"
...
metaData = {
    ...
    "argumentdetails": {
        "use_timelapse_settings": {
            ...
            "help": "When checked, uses the timelapse settings from the Allsky Settings tab instead of the values below",
            "tab": "Video",
            "type": {
                "fieldtype": "checkbox"
            }
        ...
        "generate_test_data": {
            ...
            "help": "Intended to speed up Video testing. Copy on ehour of last night's data (11PM-12AM) to ~/allsky/images/test folder. Uncheck this after generation",
            "tab": "Testing",
            "type": {
                "fieldtype": "checkbox"
            }
        ...
        "test_mode": {
            ...
            "help": "Use test folder instead of last nights image folder. Quicker for getting setup!",
            "tab": "Testing",
            "type": {
                "fieldtype": "checkbox"
            }
...
def debug_log(message, level=1, force_print=False, debug_only=False):
    """Enhanced debug logging with multiple levels and debug mode control
    
    Args:
        message (str): The message to log
        level (int): Log level (0=ERROR, 1=INFO, 2=DEBUG)
        force_print (bool): Whether to print to console regardless of debug setting
        debug_only (bool): Only log if debug mode is enabled
    """
    try:
        debug_mode = str(s.params.get("debug_mode", "false")).lower() == "true"
        
        if debug_only and not debug_mode:
            return
            
        prefix = "ERROR" if level == 0 else "INFO" if level == 1 else "DEBUG"
        log_message = f"KEOLAPSE {prefix}: {message}"
        
        if force_print or level == 0:
            print(log_message)
        
        s.log(level, f"KEOLAPSE: {message}")
...
def get_timelapse_settings():
    """Get timelapse settings from Allsky settings.json file"""
    try:
        import json
        settings_path = os.path.join(s.getEnvironmentVariable("ALLSKY_CONFIG", fatal=False) or "/home/pi/allsky/config", "settings.json")
        
        # Check if file exists
        if not os.path.exists(settings_path):
            debug_log(f"Settings file not found: {settings_path}", level=0)
            return None
        ...
        timelapse_settings = {
            "width": settings.get("timelapsewidth", 0),
            "height": settings.get("timelapseheight", 0),
            "bitrate": settings.get("timelapsebitrate", 5000),
            "fps": settings.get("timelapsefps", 12),
            "vcodec": settings.get("timelapsevcodec", "libx264"),
            "pixfmt": settings.get("timelapsepixfmt", "yuv420p"),
            "fflog": settings.get("timelapsefflog", "warning")
        }
        
        debug_log(f"Loaded timelapse settings: {timelapse_settings}", level=1)
        return timelapse_settings
        
    except Exception as e:
        debug_log(f"Error loading timelapse settings: {str(e)}", level=0)
        return None
...
def get_target_date(params):
    ...
    if process_date:
        try:
            ...
            debug_log(f"Using specified date: {process_date}", level=1)
            return process_date
        except ValueError:
            debug_log(f"Invalid date format: {process_date}, using yesterday", level=0)
...
def get_source_directory(params):
    ...
    if not os.path.exists(source_dir):
        debug_log(f"FATAL ERROR: Directory not found: {source_dir}", level=0, force_print=True)
...
def get_source_images(params):
    ...
    try:
        ...
        debug_log(f"Searching for images in: {source_dir}", level=2, debug_only=True)
        ...
        if not images:
            debug_log("No images found", level=0)
            return []
        ...
        debug_log(f"Found {len(images)} images", level=1)
        debug_log(f"Estimated video length: {minutes}:{seconds:02d} at {fps} fps", level=1)
        
        return images
        
    except Exception as e:
        debug_log(f"Failed to get source images: {str(e)}", level=0)
        return []
...
def get_keogram_path(params):
    ...
    debug_log(f"Searching for keogram in: {keogram_dir}", level=2, debug_only=True)
    
    # If we only want to show debug circles, we don't need a keogram
    if str(params.get("show_circles_only", "false")).lower() == "true":
        debug_log("Debug circles only mode - no keogram needed", level=1)
        return True
    
    # Check if keogram directory exists
    if not os.path.exists(keogram_dir):
        debug_log(f"Keogram directory not found: {keogram_dir}", level=0)
        return None
        
    # Look for keogram files
    keogram_files = glob.glob(os.path.join(keogram_dir, "keogram*.jpg"))
    if keogram_files:
        debug_log(f"Using keogram: {os.path.basename(keogram_files[0])}", level=1)
        return keogram_files[0]
    
    debug_log("No keogram found", level=0)
    return None
...
def get_output_path(params, filename):
    ...
    if source_dir is None:
        ...
        debug_log("Using fallback output path due to missing source directory", level=0)
    ...
    try:
        ...
        if os.path.exists(test_dir):
            ...
            if test_images and test_date_is_current(test_images[0], yesterday):
                debug_log("Test data is current", level=1)
                return True
        ...
        if not night_images:
            debug_log("No night images found", level=0)
            return False
        ...
        debug_log(f"Generated test data with {len(night_images)} images", level=1)
        return True
...
def cleanup_test_data(event):
    ...
    if event == "nightday":
        try:
            ...
            if os.path.exists(test_dir):
                shutil.rmtree(test_dir)
                debug_log("Cleaned up test data", level=1)
        except Exception as e:
            debug_log(f"Error cleaning test data: {str(e)}", level=0)
...
def create_debug_image(images, params):
    ...
    try:
        ...
        if frame is None:
            debug_log(f"Failed to load test image: {test_image}", level=0)
            return False
        ...
        if str(params.get("show_circles_only", "false")).lower() == "true":
            ...
            debug_log(f"Created debug circles image: {output_path}", force_print=True)
            return True
        ...
        cv2.imwrite(output_path, result)
        debug_log(f"Created debug image: {output_path}", force_print=True)
        
        return True
        
    except Exception as e:
        debug_log(f"Error creating debug image: {str(e)}", level=0)
        return False
...
def optimize_video_params(images, params):
    ...
    debug_log(f"Initial duration: {current_duration:.1f} seconds", level=2, debug_only=True)
    ...
    debug_log(f"Optimization results:", level=1)
    debug_log(f"- Original frames: {total_frames}", level=1)
    debug_log(f"- Optimized frames: {len(optimized_images)}", level=1)
    debug_log(f"- Frames kept: 1 in every {skip_interval}", level=2, debug_only=True)
    debug_log(f"- Final duration: {len(optimized_images)/optimized_fps:.1f} seconds", level=1)
...
def should_optimize_video(images, params):
    ...
    # Check if using timelapse settings
...
def calculate_scaled_dimensions(params, original_width, original_height):
    ...
    if original_height < target_height:
        ...
        debug_log(f"Maintaining original resolution ({original_height}p)", level=1)

    scale_factor = target_height / original_height
    new_width = int(original_width * scale_factor)
    
    debug_log(f"Scale factor: {scale_factor:.3f}", level=2, debug_only=True)
    debug_log(f"New dimensions: {new_width}x{target_height}", level=2, debug_only=True)
...
class KeolapseGenerator:
        ...
        try:
            ...
            if expansion_needed:
                debug_log(f"Expanding image to fit keogram ring: top={self.expansion['top']}, " +
                         ...
                         f"right={self.expansion['right']}", level=1)
            ...
            debug_log(f"Circle detection: center=({center_x}, {center_y}), radius={radius}", 
                     level=2, debug_only=True)
    ...
    def wrap_keogram(self, base_image, keogram, progress_indicator, frame_idx, total_frames):
        ...
        try:
            ...
            if str(self.params.get("show_circles_only", "false")).lower() == "true":
            ...
            if str(self.params.get("show_circles", "false")).lower() == "true":
                cv2.circle(result, self.circle_center, self.circle_radius,
            ...
            if str(self.params.get("show_circles", "false")).lower() == "true" or \
            str(self.params.get("show_example", "false")).lower() == "true":
...
def create_video(images, params):
    ...
    if not images:
        debug_log("No images found to process", level=0)
        return False
    ...
    try:
        ...
        # Get timelapse settings if enabled
        use_timelapse_settings = str(params.get("use_timelapse_settings", "false")).lower() == "true"
        ...
        if use_timelapse_settings:
            timelapse_settings = get_timelapse_settings()
            if timelapse_settings:
                debug_log("Using timelapse settings from Allsky Settings", level=1)
            else:
                debug_log("Failed to load timelapse settings, using module settings instead", level=0)
                use_timelapse_settings = False
        ...
        if use_timelapse_settings and timelapse_settings:
            ...
            debug_log(f"Using timelapse resolution: {new_width}x{new_height}", level=1)
        else:
            # Use module settings
            new_width, new_height, scale_factor = calculate_scaled_dimensions(
                params, original_width, original_height)
        ...
        # Check if we're in circles-only mode
        circles_only = str(params.get("show_circles_only", "false")).lower() == "true"
        ...
        if not circles_only:
            ...
            debug_log("Using circles-only mode (no keogram overlay)", level=1)
        ...
        debug_log(f"Final frame dimensions: {width}x{height}", level=1)
        
        # Apply video optimization if needed
        if should_optimize_video(images, params):
            debug_log("Video length exceeds maximum, optimizing...", level=1)
        ...
        debug_log(f"Output video dimensions: {width}x{height} @ {fps}fps", level=1)
        ...
        if use_timelapse_settings and timelapse_settings:
            ...
            debug_log(f"Using timelapse quality settings (bitrate: {bitrate}k)", level=1)
        else:
            quality_setting = params.get("video_quality", "medium")
            quality_params = VIDEO_QUALITY.get(quality_setting, VIDEO_QUALITY["medium"])
            debug_log(f"Using video quality: {quality_setting}", level=1)
            
        debug_log(f"Quality parameters: {quality_params}", level=2, debug_only=True)
        ...
        for i, img_path in enumerate(images):
            ...
            if i % progress_interval == 0:
                ...
                if i > 0:
                    ...
                    debug_log(f"Progress: {progress_pct:.1f}% ({i+1}/{total_frames})", level=1)
            
            # Process frame
            frame = cv2.imread(img_path)
            if frame is None:
                debug_log(f"Failed to read frame: {os.path.basename(img_path)}", 
                         level=2, debug_only=True)
                continue
        ...
        try:
            ...
            debug_log("Re-encoding video with quality settings...", level=1)
            ...
            debug_log(f"Failed to apply quality settings: {str(e)}", level=0)
            # If re-encoding fails, keep the original file
            if os.path.exists(temp_output):
                os.rename(temp_output, output_path)
                debug_log("Using original encoded video (ffmpeg failed)", level=0)
        
        # Log completion
        total_time = (datetime.now() - start_time).total_seconds()
        time_str = f"{total_time/60:.1f} minutes" if total_time > 60 else f"{total_time:.1f} seconds"
        debug_log(f"Video creation completed in {time_str}", level=1)
...
def keolapse(params, event):
    ...
    debug_log("=== Starting Keolapse Module ===", level=1)
    debug_log(f"Base directory: {base_dir}", level=2, debug_only=True)
    debug_log(f"Event: {event}", level=2, debug_only=True)
    debug_log(f"Module version: {metaData['version']}", level=1)
    debug_log(f"Testing mode: {'Enabled' if testing_enabled else 'Disabled'}", level=1)
    ...
    try:
        ...
        if testing_enabled and event == "periodic" and str(params.get("generate_test_data", "false")).lower() == "true":
            debug_log("Generating test data...", level=1)
        ...
        if testing_enabled and event == "periodic" and (
            ...
            str(params.get("show_circles_only", "false")).lower() == "true"
        ):
            debug_log("Creating debug image...", level=1)
        ...
        debug_log("Creating keolapse video...", level=1)
...
def keolapse_cleanup():
    ...
    try:
        ...
        print(f"KEOLAPSE ERROR during cleanup: {str(e)}")